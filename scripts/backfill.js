// scripts/backfill.js
// Manual historical collector - run from .github/workflows/backfill.yml
// Deliberately kept separate from fetch.js: that one runs unattended every 4 hours,
// this one runs by hand over a wide range with different skip rules. Duplicated
// helpers are the price of the scheduled job never breaking because of a change here.
//
...
// Day boundaries in IST - must match fetch.js and cleanup.js, since filenames,
// index entries and retention arithmetic all key off it.
...
// published explicitly in data/index.json. Mirrors fetch.js.
...
// DAYS_BACK past days through to 2 days ahead, oldest first
function buildDates() {
...
async function fetchOneHD(tag, date, today) {
  ...
  // Past days never change, so re-running a backfill is cheap. Today and future
  // dates are always re-fetched.
